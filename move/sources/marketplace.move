...
public fun buy_hero(list_hero: ListHero, coin: Coin<SUI>, ctx: &mut TxContext) {
    ...
    // ListHero yapısını parçalara ayır
    let ListHero {id, nft, price, seller} = list_hero;

    // ÖDEME KONTROLÜ (DÜZELTİLDİ):
    // Testlerin geçmesi için ödemenin TAM OLARAK fiyata eşit olması gerekir.
    // Eğer (value != price) ise işlem EInvalidPayment (1) hatası ile durur.
    assert!(coin::value(&coin) == price, EInvalidPayment);

    // Coin'i satıcıya transfer et
    transfer::public_transfer(coin, seller);

    // Hero NFT'sini alıcıya (ctx.sender()) transfer et
...
public fun delist(_: &AdminCap, list_hero: ListHero) {
    ...
    // 'price' kullanılmadığı için '_' ile atlandı
    let ListHero {id, nft, price: _, seller} = list_hero;

    // NFT'yi orijinal satıcıya geri gönder
